# -*- coding: utf-8 -*-
"""
Created on Sun Mar  8 19:35:10 2020
...
def par_mac(str):
    ...
    if (1):
        ...
        matchObj_id = re.search(r"N°\d{1,3}", str, re.M | re.I)
        if (matchObj_id != None):
            dict_name["id"] = matchObj_id.group()
            print(matchObj_id.group())
        matchObj_name = re.search(r"</span> </span>.{1,}</a> ", str, re.M | re.I)
        if (matchObj_name != None):
            length = len(matchObj_name.group().split(">")[2].split("<")[0].split())
            strnew = ""
            for i in range(1, length):
                strnew = strnew + matchObj_name.group().split(">")[2].split("<")[0].split()[i] + " "
            print(strnew)
            dict_name["name"] = strnew
        else:
            print("No name")
        #    print(dict_name)
        #    except Exception:
